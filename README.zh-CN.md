...
4. 打开 xPoster 侧边栏。
5. 直接在编辑框里粘贴 Markdown，或者选择 `.md` 文件。
   单篇草稿会留在编辑框里，方便继续修改。
6. 看预览和问题列表。
7. 点击 **Check article / 检查文章**。
8. 点击 **Write article / Import / 写入文章 / 导入**。如果一次选择或拖入多篇 Markdown，编辑框会替换成待发布列表；点击某一行可在弹出框里编辑，底部按钮会批量逐篇写入，弹出框里也可以只写入当前这一篇。
9. 回到 X 文章编辑器里仔细检查。
10. 确认没问题后，再手动点击 X 自己的发布按钮。

## 待发布草稿

待发布页默认是一个 Markdown 编辑框和下面的写入按钮。只有一次选择或拖入多篇 `.md` 时，编辑框才会替换成紧凑列表。点击列表里的任意一篇，会用弹出框编辑这篇 Markdown。底部按钮会一键逐篇写入队列；弹出框里也可以只写入当前这一篇。某篇成功写入 X Article 后，会立刻从队列移除。
