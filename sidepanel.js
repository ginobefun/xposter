...
\`\`\``;
  ...
  const ZH_TEXT = new Map(
    Object.entries({
      ...
      "it checks every image first. If one COS link is temporarily unreachable, import stops and names the failed image.": "它会先检查每张图片。如果某个 COS 链接暂时不可访问，导入会停止并标出失败图片。",
      ...
      "Click Allow image website. When Chrome asks for example-image-host.invalid, choose Allow. Then click Check downloads before Import.": "点击允许图片网站。当 Chrome 询问 example-image-host.invalid 时，选择允许。然后导入前点击检查下载。",
      ...
      "Click Check images before importing a multi-image draft. xPoster will list the exact image if a COS URL is temporarily unreachable.": "导入多图草稿前请点击检查图片。如果某个 COS URL 暂时不可访问，xPoster 会列出具体图片。",
